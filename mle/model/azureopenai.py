...
class AzureOpenAIModel(Model):
    def __init__(self, api_key, model, temperature=0.7):
        ...
        dependency = "azure.ai.openai"
        spec = importlib.util.find_spec(dependency)
        if spec is not None:
            self.openai = importlib.import_module(dependency).AzureOpenAI
        else:
            raise ImportError(
                "It seems you didn't install azure.ai.openai. In order to enable the OpenAI client related features, "
                "please make sure azure.ai.openai Python package has been installed. "
                "More information, please refer to: https://aka.ms/azure-openai"
            )
